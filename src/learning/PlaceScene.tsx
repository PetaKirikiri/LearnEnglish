...
export default function PlaceScene({ relation }: { relation: PlaceRelation }) {
  ...
  const ball = (x: number, y: number, r = 18) => <g data-object="ball"><circle cx={x} cy={y} r={r} fill={`url(#${id}-ball)`} stroke="#f4dfad" strokeWidth="1.5"/><path d={`M ${x-r*.55} ${y-r*.2} Q ${x} ${y-r*.8} ${x+r*.45} ${y-r*.4}`} fill="none" stroke="#fff1cd" strokeWidth="2" opacity=".65"/></g>
  ...
  return <svg viewBox="0 0 360 156" role="img" aria-label={descriptions[relation]} lang="th" style={{ display:'block', width:'100%', height:'clamp(110px, 18vh, 156px)', margin:'12px auto 0' }}>
    ...
    <path d="M48 139 H312" stroke="#8bacc8" opacity=".2"/>
