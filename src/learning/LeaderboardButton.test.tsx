...
const rows = [
  { id: 'fifa', name: 'Fifa', rank: 1, points: 100 },
...
it('shows the champion beside the header cup and opens every member without losing the question', async () => {
  ...
  expect(button.querySelector('svg')!.nextElementSibling!.textContent).toBe('Fifa')
...
it('does not invent a champion when every member has zero points', async () => {
  ...
  await render()
  expect(container.querySelector('.champion-button span')).toBeNull()
