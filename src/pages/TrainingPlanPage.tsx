  ...
  return (
    <section className="mx-auto max-w-6xl space-y-6">
      <header className="rounded-3xl bg-white p-6 sm:p-8">
        ...
        <details className="mt-5 rounded-xl border border-slate-200 p-4 text-sm leading-6"><summary className="cursor-pointer font-bold">What the live app does today</summary><p className="mt-2">Each round contains 10 questions. It first includes up to five previously missed questions, then adds a mix from different stories. It does not yet follow frequency order or require every sentence for a word before advancing.</p><p className="mt-2">Three correct rounds per existing question are recorded today. Thai sentence guidance, a review of ambiguous answer choices, and the later-session check are still needed for this proposed path.</p></details>
      </header>

      <div className="grid gap-3 sm:grid-cols-3">
        {[['Word priorities', plan.length, 'Ordered by frequency; alphabetical when tied'], ['Words with a current sentence exercise', coverage, 'Coverage only; answer choices still need review'], ['Words without a current sentence exercise', plan.length - coverage, 'Shown in the path, never silently skipped']].map(([label, count, note]) => <div key={label} className="rounded-2xl bg-white p-5"><p className="text-sm font-bold text-slate-600">{label}</p><strong className="mt-2 block text-3xl">{count}</strong><p className="mt-2 text-xs leading-5 text-slate-500">{note}</p></div>)}
      </div>
      ...
      <div className="grid items-start gap-6 lg:grid-cols-[minmax(280px,1fr)_minmax(0,2fr)]">
        ...
        <section aria-label="Selected word training" className="min-w-0 space-y-4">
          ...
          <p className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-950">{selected.examples.length === 0 ? 'This word occurs in a title, heading, or other text without a complete sentence. Add a suitable sentence before teaching it.' : tested < selected.examples.length ? `${selected.examples.length - tested} examples still need exercises targeting “${selected.word}”. ` : 'Every source example has a current exercise. '}Existing exercises need their meaning and answer choices checked before they count toward this proposed path. The later-session check is not implemented.</p>
          {selected.examples.slice(examplePage * 10, (examplePage + 1) * 10).map((example, index) => {
            ...
            return <article key={`${selected.word}:${example.id}`} className="rounded-2xl border border-slate-200 bg-white p-5">
              ...
              {example.questions.map(question => <details key={question.id} className="mt-4 rounded-xl border border-slate-200 p-4 text-sm"><summary className="cursor-pointer font-bold">Inspect current exercise{loaded ? ` · ${Math.min(3, records.get(question.id)?.correctRounds.size ?? 0)} / 3 correct rounds` : ''}</summary><p className="mt-3 text-base font-bold">{question.prompt}</p><div className="mt-3 flex flex-wrap gap-2">{question.choices.map(choice => <span key={choice} className={`rounded-lg border px-3 py-2 ${choice === question.answer ? 'border-blue-300 bg-blue-50 font-bold' : 'border-slate-200'}`}>{choice}{choice === question.answer ? ' · stored answer' : ''}</span>)}</div><p className="mt-3 text-xs leading-5 text-amber-800">The stored answer matches the story. That alone does not prove that the other choices are grammatically wrong.</p></details>)}
            </article>
          })}
