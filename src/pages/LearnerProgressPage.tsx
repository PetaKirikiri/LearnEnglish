    ...
    <section className="mx-auto max-w-5xl space-y-6">
      ...
      {learnerId && (!loading || updated) && <>
        ...
        <section className="rounded-3xl bg-white p-6">
          ...
          <details className="mt-3 text-xs leading-6 text-slate-500"><summary className="w-fit cursor-pointer font-semibold">Details</summary><p className="mt-3 text-sm leading-6 text-slate-500">“Learned” means correct in three separate rounds since the last mistake. It measures performance on the current exercises. The stories contain {words.length} different words; {catalogue.vocabulary.length} are currently in vocabulary lessons.</p></details>
        ...
        <div className="grid gap-6 sm:grid-cols-2">
          <section className="rounded-3xl bg-white p-6"><h2 className="text-xl font-black">App use</h2><dl className="mt-5 grid grid-cols-2 gap-5">{[['App visits', summary.visits], ['Sign-ins', summary.logins], ['Active minutes', Math.floor(summary.seconds / 60)], ['Rounds completed', summary.rounds]].map(([label, value]) => <div key={label}><dt className="text-sm text-slate-500">{label}</dt><dd className="mt-1 text-2xl font-bold">{value}</dd></div>)}</dl><details className="mt-3 text-xs leading-6 text-slate-500"><summary className="w-fit cursor-pointer font-semibold">Details</summary><p className="mt-5 text-xs leading-5 text-slate-500">Visits count opening the student app; sign-ins count successful logins. Active time is approximate and pauses when the page is hidden or idle for a minute. Tracking began: {showDate(summary.firstTracked)}.</p></details></section>
        ...
        <details className="mt-3 text-xs leading-6 text-slate-500"><summary className="w-fit cursor-pointer font-semibold">Details</summary><p className="text-xs leading-5 text-slate-500">Earlier answer totals are imported once when that learner opens the updated app in the browser where they practised. Their original round history is unknown, so it cannot establish mastery. Activity on another phone appears after it syncs.</p></details>
      </>}
    </section>
  )
}
