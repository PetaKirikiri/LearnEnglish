...
export default function QuestionsTablePage() {
  ...
  return <section>
    ...
    <p className="sheet-note">{view==='bank' ? 'Every current question, highest-frequency target words first. # stays fixed when filtering. This is priority order, not a fixed play sequence.' : 'Exact base round from the current app, without a learner’s review queue. Personal mistakes can change the order.'}</p>
